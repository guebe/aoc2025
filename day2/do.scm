...
(define (expand-range range)
  (let ((start (car range))
	(end (cadr range)))
    (iota (- (+ end 1) start) start)))

(define part1 "^(.+)\\1$")
(define part2 "^(.+)\\1{1,}$")

(define (solve rgx ranges)
  (fold + 0 (map string->number (filter (lambda (s) (irregex-match? rgx s)) (map number->string (append-map expand-range ranges))))))

(display (solve part1 example))
(newline)
(display (solve part1 input))
(newline)
(display (solve part2 example))
(newline)
(display (solve part2 input))
(newline)
